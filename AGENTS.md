...
* テストコード（`test/wasm_engine_test.cpp` など）で使用する正常系テスト用のWASMバイナリ配列は、手動で適当に組み立てたものではなく、必ず `clang` で生成したWASM仕様に完全に準拠したバイト配列を使用してください。バイナリ配列を生成・更新する際は、`tools/codegen/generate_test_wasms.py` などの自動生成スクリプトを利用してください。
