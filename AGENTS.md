...
* To build and run the official WASM spec tests, you must pass `-DEMBWASM_BUILD_OFFICIAL_SPEC_TESTS=ON` as a CMake option. Without this flag, the spec test targets are excluded from the build. Example: `cmake -DEMBWASM_BUILD_OFFICIAL_SPEC_TESTS=ON ..`
