...
* The CMake build output directory is `cmake-build-debug/`. Always run `cmake --build` targeting that directory. Example: `cmake --build cmake-build-debug`
* To build and run the official WASM spec tests, you must pass `-DEMBWASM_BUILD_OFFICIAL_SPEC_TESTS=ON` as a CMake option. Without this flag, the spec test targets are excluded from the build. Example: `cmake -B cmake-build-debug -DEMBWASM_BUILD_OFFICIAL_SPEC_TESTS=ON && cmake --build cmake-build-debug`
