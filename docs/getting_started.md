...
```mermaid
graph TD
    A[1. api_config.yaml にインポート関数を定義] --> B[2. wasm_host_apis.h にC++前方宣言を追加]
...
* **[api_config.yaml](file:///Users/nabeshimamasataka/CLionProjects/embwasm/api_config.yaml)** の `headers` セクションに任意のヘッダーファイル名を追加し、`apis` セクションに対応関係を記述します。
